    ...
    def read_queries(self, file_path, is_train=False):
        ...
        with open(file_path, 'r', encoding='UTF-8') as f:
            for line in f.readlines():
                ...
                easy_answers = [] if easy_answers.strip() == '' else [int(i) for i in easy_answers.strip().split(' ')]
                hard_answers = [] if hard_answers.strip() == '' else [int(i) for i in hard_answers.strip().split(' ')]
